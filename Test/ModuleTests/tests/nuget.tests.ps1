...
Describe "Event Test" -Tags "Feature" {
    ...
    it "EXPECTED: install a package should report destination" -Skip:(-not $WindowsPowerShell) {
        ...
        while($retryCount -gt 0)
        {
            $events = @(Get-WinEvent -FilterHashtable @{ ProviderName = $providerName; Id = 4101 } -ErrorAction SilentlyContinue -WarningAction SilentlyContinue)
...
Describe "Install-Package dependencies" -Tags "Feature" {
    ...
    It "Install package with unlisted dependencies" {
        ...
        (Test-Path "$tempDir\PMTestModule.1.0.3") | should be $true
