...
Describe "import-packageprovider" -Tags "Feature" {
    ...
    It "Import a PowerShell package provider Expect succeed" -Skip:($IsCoreCLR){
    ...
    It "Import 'PSChained1Provider' PowerShell package provider with filePath from programfiles folder, Expect succeed" -Skip:($IsCoreCLR){
    ...
    It "Import a PowerShell package provider with filePath from user folder -force, Expect succeed" -Skip:($IsCoreCLR){
    ...
    It "Import a PowerShell package provider with -force, Expect succeed" -Skip:($IsCoreCLR){
    ...
    It "Import 'OneGetTest' PowerShell package provider that has multiple versions, Expect succeed" -Skip:($IsCoreCLR){
...
Describe "import-packageprovider Error Cases" -Tags "Feature" {
  ...
  It "EXPECTED:  returns an error when asking for a provider that does not exist" {
   ...
   It "EXPECTED:  returns an error when asking for a provider with RequiredVersoin and MinimumVersion" {
        $Error.Clear()
        import-packageprovider -name PowerShellGet -RequiredVersion 1.0 -MinimumVersion 2.0  -warningaction:silentlycontinue -ea silentlycontinue -force
   ...
   It "EXPECTED:  returns an error when asking for a provider with RequiredVersoin and MaximumVersion" {
        $Error.Clear()
        import-packageprovider -name PowerShellGet -RequiredVersion 1.0 -MaximumVersion 2.0  -warningaction:silentlycontinue -ea silentlycontinue -force
        $ERROR[0].FullyQualifiedErrorId | should be "VersionRangeAndRequiredVersionCannotBeSpecifiedTogether,Microsoft.PowerShell.PackageManagement.Cmdlets.ImportPackageProvider"
    }

   It "EXPECTED:  returns an error when asking for a provider with a MinimumVersion greater than MaximumVersion" {
        $Error.Clear()
        import-packageprovider -name PowerShellGet -MaximumVersion 1.0 -MinimumVersion 2.0 -warningaction:silentlycontinue -ea silentlycontinue -force
   ...
   It "EXPECTED:  returns an error when asking for a provider with MinimumVersion that does not exist" {
        $Error.Clear()
        Import-packageprovider -name OneGetTest -MinimumVersion 20.2 -warningaction:silentlycontinue -ea silentlycontinue -force
        $ERROR[0].FullyQualifiedErrorId | should be "NoMatchFoundForCriteria,Microsoft.PowerShell.PackageManagement.Cmdlets.ImportPackageProvider"
    }

   It "EXPECTED:  returns an error when asking for a provider with MaximumVersion that does not exist" {
        $Error.Clear()
        Import-packageprovider -name OneGetTest -MaximumVersion 0.2 -warningaction:silentlycontinue -ea silentlycontinue -force
        $ERROR[0].FullyQualifiedErrorId | should be "NoMatchFoundForCriteria,Microsoft.PowerShell.PackageManagement.Cmdlets.ImportPackageProvider"
    }
...
Describe "Import-PackageProvider with OneGetTest that has 3 versions: 1.1, 3.5, and 9.9." -Tags "Feature" {
    ...
    It "EXPECTED: success 'import OneGetTest -requiredVersion 3.5'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'Import OneGetTest -requiredVersion 3.5 and then 9.9 -force'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'import OneGetTest with MinimumVersion and MaximumVersion'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'OneGetTest with MaximumVersion'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'OneGetTest with MinimumVersion'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'OneGetTest Find-Package with Progress'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'OneGetTest Find-Package returns correct TagId'" -Skip:($IsCoreCLR){
        $ps = [PowerShell]::Create()
        $ps.AddScript("Import-PackageProvider -Name OneGetTest -RequiredVersion 9.9; Find-Package -ProviderName OneGetTest")
    ...
    It "EXPECTED: success 'OneGetTest Get-Package returns correct package object using swidtag'" -Skip:($IsCoreCLR) {
        $ps = [PowerShell]::Create()
    ...
    It "EXPECTED: success 'OneGetTest Get-Package returns correct progress message'" -Skip:($IsCoreCLR) {
        $ps = [PowerShell]::Create()
        $null = $ps.AddScript("`$null = Import-PackageProvider -Name OneGetTest -RequiredVersion 9.9; Get-Package -ProviderName OneGetTest")
        $result = $ps.Invoke() 
...
Describe "Import-PackageProvider with OneGetTestProvider that has 2 versions: 4.5, 6.1" -Tags "Feature" {
    ...
    It "EXPECTED: Get-PackageProvider -ListAvailable succeeds" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: Get-PackageProvider -ListAvailable succeeds even after importing gist provider" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'import OneGetTestProvider -requiredVersion 4.5'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'import OneGetTestProvider with MinimumVersion and MaximumVersion'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'import OneGetTestProvider with MaximumVersion'" -Skip:($IsCoreCLR){
        Import-packageprovider -name OneGetTestProvider -MaximumVersion 4.6 -Force
        (Get-PackageProvider OneGetTestProvider).Version.ToString() | should match '4.5.0.0'
    }
    
    It "EXPECTED: success 'OneGetTestProvider with MinimumVersion'" -Skip:($IsCoreCLR){
    ...
    It "EXPECTED: success 'Import OneGetTestProvider -requiredVersion 4.5 and then 6.1 -force'" -Skip:($IsCoreCLR){
        Import-PackageProvider -Name OneGetTestProvider -RequiredVersion 4.5 -Force;
        Import-PackageProvider -Name OneGetTestProvider -RequiredVersion 6.1 -Force;
        (Get-PackageProvider OneGetTestProvider).Version.ToString() | should match '6.1.0.0'
    }

    It "EXPECTED: success 'Import OneGetTestProvider -MinimumVersion 4.5 and then MaximumVersion 5.0 -force'" -Skip:($IsCoreCLR) {
