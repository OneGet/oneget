...
namespace Microsoft.OneGet.Test.Core.Service {
    ...
    public class ArpProviderTest : BasePMSServiceTests {
        public ArpProviderTest(ITestOutputHelper outputHelper)
            : base(outputHelper) {
        }
        ...
        public PackageProvider Provider {
            get {
                if (_provider == null) {
                    var packageProviders = PackageManagementService.SelectProviders("arp", new BasicHostImpl()).ToArray();
