...
namespace Microsoft.PowerShell.PackageManagement.Cmdlets
{
    ...
    public abstract class AsyncCmdlet : PSCmdlet, IDynamicParameters, IDisposable
    {
        ...
        private ManualResetEvent ReentrantLock =>
                ...
                GetType().GetOrAdd(() => new ManualResetEvent(false), "ReentrancyLock");
        ...
        protected bool Confirm => MyInvocation.BoundParameters.ContainsKey(Constants.Parameters.ConfirmParameter) && (SwitchParameter)MyInvocation.BoundParameters[Constants.Parameters.ConfirmParameter];

        public bool WhatIf => MyInvocation.BoundParameters.ContainsKey(Constants.Parameters.WhatIfParameter) && (SwitchParameter)MyInvocation.BoundParameters[Constants.Parameters.WhatIfParameter];
        ...
        protected bool IsInvocation =>
                ...
                MyInvocation != null && MyInvocation.PipelineLength > 0;
        ...
        public bool IsCanceled => Stopping || CancellationEvent == null || CancellationEvent.IsCancellationRequested;
        ...
        protected Dictionary<string, object> UnboundArguments
        {
            get
            {
                if (_unboundArguments == null && IsReentrantLocked)
                {
                    ...
                    try
                    {
                        object context = TryGetProperty(this, "Context");
                        ...
                        if (TryGetProperty(parameterBinder, "UnboundArguments") is IEnumerable args)
                        {
                            ...
                            foreach (object arg in args)
                            {
                                ...
                                {
                                    object parameterName = TryGetProperty(arg, "ParameterName");
                                    ...
                                    if (parameterName != null)
                                    {
                                        ...
                                        _unboundArguments.AddOrSet(currentParameterName, TryGetProperty(arg, "ArgumentValue"));
                                        continue;
                                    }
                                }
        ...
        {
            string[] files = GetResolvedProviderPathFromPSPath(filePath, out ProviderInfo providerInfo).ToArray();
            switch (files.Length)
            {
                case 0:
                    // none found
        ...
        public bool Error(string id, string category, string targetObjectValue, string messageText, params object[] args)
        {
            if (IsInvocation)
            {
                string errorMessage = FormatMessageString(messageText, args);
        ...
        public bool Debug(string messageText, params object[] args)
        {
            if (IsInvocation)
            {
                ...
                Task<bool> task = WriteDebug("{0} {1}".format(_stopwatch.Elapsed, FormatMessageString(messageText, args)));
#if WAIT_FOR_DEBUG
        ...
        public bool Progress(int activityId, int progressPercentage, string messageText, params object[] args)
        {
            lock (_progressTrackers)
            {
                if (IsInvocation)
                {
                    ProgressTracker p = _progressTrackers.FirstOrDefault(each => each.Id == activityId);
                    if (p != null)
                    {
                        if (progressPercentage >= 100)
                        {
            ...
            lock (_progressTrackers)
            {
                if (IsInvocation)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        List<ProgressTracker> cloneChildrenList = new List<ProgressTracker>(p.Children);

                        // complete all of this trackers kids.
                        foreach (ProgressTracker child in cloneChildrenList)
                        {
        ...
        public virtual RuntimeDefinedParameterDictionary DynamicParameterDictionary => _dynamicParameters ?? (_dynamicParameters = new RuntimeDefinedParameterDictionary());
        ...
        protected T GetDynamicParameterValue<T>(string parameterName)
        {
            ...
            {
                RuntimeDefinedParameter p = DynamicParameterDictionary[parameterName];
                if (p.IsSet)
                {
        ...
        private void ProcessHeldMessages()
        {
            ...
            {
                foreach (TaskCompletionSource<bool> msg in _heldMessages.GetConsumingEnumerable().Where(msg => msg != null))
                {
                    InvokeMessage(msg);
                }
        ...
        private void AsyncRun(Func<bool> asyncAction)
        {
            try
            {
                using (_messages = new BlockingCollection<TaskCompletionSource<bool>>())
                {
                    ...
                    foreach (TaskCompletionSource<bool> message in _messages.GetBlockingEnumerable(CancellationEvent.Token))
                    {
        ...
        protected bool IsProcessing => CmdletState == AsyncCmdletState.BeginProcess || CmdletState == AsyncCmdletState.ProcessRecord || CmdletState == AsyncCmdletState.EndProcess;

        protected bool IsBeforeProcessing => CmdletState < AsyncCmdletState.BeginProcess;

        protected bool IsAfterProcessing => CmdletState > AsyncCmdletState.EndProcess;
        ...
        {
            TaskCompletionSource<bool> message = new TaskCompletionSource<bool>(onMainThreadDelegate);
        ...
        #endregion PowerShell response streams
        ...
        {
            ShouldContinueResult shouldContinueResult = new ShouldContinueResult
            {
                yesToAll = false,
                noToAll = false,
                result = false
        ...
        #endregion CmdLet Interactivity
        ...
        protected object TryGetProperty(object instance, string fieldName)
        {
            ...
            PropertyInfo propertyInfo = instance.GetType().GetProperty(fieldName, BindingFlags);
        ...
        #endregion Direct Property Access
    }
}