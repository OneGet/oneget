...
namespace Microsoft.PackageManagement.Internal.Implementation
{
    ...
    internal class ProviderServicesImpl : IProviderServices
    {
        ...
        public bool IsElevated => AdminPrivilege.IsElevated;
        ...
        public string DownloadFile(Uri remoteLocation, string localFilename, int timeoutMilliseconds, bool showProgress, IRequest request)
        {
            ...
            if (!request.IsCanceled)
            {
                ...
                foreach (Downloader downloader in PackageManagementService.Downloaders.Values)
                {
        ...
        public IEnumerable<string> UnpackArchive(string localFilename, string destinationFolder, IRequest request)
        {
            ...
            if (!request.IsCanceled)
            {
                ...
                foreach (Archiver archiver in PackageManagementService.Archivers.Values)
                {
        ...
        public bool Install(string fileName, string additionalArgs, IRequest request)
        {
            ...
            if (!request.IsCanceled)
            {
                ...
                foreach (PackageManagement.Implementation.PackageProvider provider in PackageManager.Instance.PackageProviders)
                {
                    SoftwareIdentity[] packages = provider.FindPackageByFile(fileName, request).ToArray();
                    if (packages.Length > 0)
                    {
                        ...
                        foreach (SoftwareIdentity package in packages)
                        {
                            foreach (SoftwareIdentity installedPackage in provider.InstallPackage(package, request.Extend<IRequest>(new
                            {
        ...
        public bool IsSignedAndTrusted(string filename, IRequest request)
        {
            ...
            if (!request.IsCanceled)
            {
                ...
                WinVerifyTrustResult result = NativeMethods.WinVerifyTrust(new IntPtr(-1), new Guid("{00AAC56B-CD44-11d0-8CC2-00C04FC295EE}"), wtd);
