...
namespace Microsoft.PackageManagement.Packaging
{
    ...
    public class SoftwareIdentity : Swidtag
    {
        ...
        {
            get => Element.Elements(Iso19770_2.Elements.Meta).Select(each =>
                                 {
                                     string summary = each.GetAttribute(Iso19770_2.Attributes.Summary);
            ...
            internal set => (Meta.FirstOrDefault() ?? AddMeta()).AddAttribute(Iso19770_2.Attributes.Summary, value);
        }
        ...
        public string CanonicalId => CreateCanonicalId(ProviderName, Name, Version, Source);

        public MetadataIndexer Metadata => new MetadataIndexer(this);
        ...
        internal string InstallationPath
        {
            get
            {
                ...
                IEnumerable<string> installationDirectories = Payload.Directories
        ...
        internal void AddMetadataAttribute(string metaKey, string value)
        {
            ...
            MutableEnumerable<XElement> metaElements = Element.Elements(Iso19770_2.Elements.Meta).ReEnumerable();
            string currentValue = metaElements.Select(each => each.GetAttribute(metaKey)).WhereNotNull().FirstOrDefault();
        ...
        {
            XElement element = FindElementWithUniqueId(elementPath);
            if (element != null)
            {
        ...
        {
            Link link = AddLink(referenceUri, relationship);
        ...
        {
            Entity entity = AddEntity(name, regid, role);
        ...
        internal string AddMetadataValue(string elementPath, string name, string value)
        {
            ...
            if (element == Element)
            {
                ...
                SoftwareMetadata meta = (Meta.FirstOrDefault() ?? AddMeta());
            ...
            if (element.Name == Iso19770_2.Elements.Entity)
            {
                ...
                Meta meta = (entity.Meta.FirstOrDefault() ?? entity.AddMeta());
                meta.AddAttribute(name, value);
                return meta.ElementUniqueId;
            }
        ...
        internal string AddProcess(string elementPath, string processName, int pid)
        {
            ...
            if (element.Name == Iso19770_2.Elements.Payload || element.Name == Iso19770_2.Elements.Evidence)
            {
                Process process = new ResourceCollection(element).AddProcess(processName);
                if (pid != 0)
                {
                    process.Pid = pid;
                }
        ...
        internal string AddFile(string elementPath, string fileName, string location, string root, bool isKey, long size, string version)
        {
            ...
            {
                File file = new ResourceCollection(element).AddFile(fileName);
        ...
        internal string AddDirectory(string elementPath, string directoryName, string location, string root, bool isKey)
        {
            ...
            if (element.Name == Iso19770_2.Elements.Payload || element.Name == Iso19770_2.Elements.Evidence || element.Name == Iso19770_2.Elements.Directory)
            {
                Directory file = new ResourceCollection(element).AddDirectory(directoryName);
                if (!string.IsNullOrWhiteSpace(location))
                {
                    file.Location = location;
                }
