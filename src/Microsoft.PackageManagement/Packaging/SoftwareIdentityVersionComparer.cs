...
namespace Microsoft.PackageManagement.Internal.Packaging
{
    ...
    public class SoftwareIdentityVersionComparer : IComparer<SoftwareIdentity>
    {
        ...
        public int Compare(SoftwareIdentity x, SoftwareIdentity y)
        {
            ...
            string xVersionScheme = x.VersionScheme ?? string.Empty;
            string yVersionScheme = y.VersionScheme ?? string.Empty;
        ...
        {
            string[] xs = xVersion.Split('.');
            string[] ys = yVersion.Split('.');
            int len = Math.Max(xs.Length, ys.Length);
            for (int i = 0; i < len; i++)
            {
                if (ulong.TryParse(xs.Length > i ? xs[i] : "0", out ulong xLong) && ulong.TryParse(ys.Length > i ? ys[i] : "0", out ulong yLong))
                {
                    int compare = xLong.CompareTo(yLong);
                    if (compare != 0)
                    {
                        return compare;
                    }
        ...
        {
            int xPos = IndexOfNonNumericWithDots(xVersion);
            int yPos = IndexOfNonNumericWithDots(yVersion);
            string xMulti = xPos == -1 ? xVersion : xVersion.Substring(0, xPos);
            string yMulti = yPos == -1 ? yVersion : yVersion.Substring(0, yPos);
            int compare = CompareMultipartNumeric(xMulti, yMulti);
            if (compare != 0)
            {
                return compare;
            }
