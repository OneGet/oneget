...
namespace Microsoft.PackageManagement.Internal.Packaging
{
    ...
    public class ResourceCollection : BaseElement
    {
        ...
        /// </summary>
        public IEnumerable<Directory> Directories => Element.Elements(Iso19770_2.Elements.Directory).Select(each => new Directory(each)).ReEnumerable();

        /// <summary>
        ///     An enumeration of the Files contained in the element
        /// </summary>
        public IEnumerable<File> Files => Element.Elements(Iso19770_2.Elements.File).Select(each => new File(each)).ReEnumerable();

        /// <summary>
        ///     An enumeration of the Process elements in the element.
        /// </summary>
        public IEnumerable<Process> Processes => Element.Elements(Iso19770_2.Elements.Process).Select(each => new Process(each)).ReEnumerable();

        /// <summary>
        ///     An enumeration of the resource elements in the element.
        /// </summary>
        public IEnumerable<Resource> Resources => Element.Elements(Iso19770_2.Elements.Resource).Select(each => new Resource(each)).ReEnumerable();
