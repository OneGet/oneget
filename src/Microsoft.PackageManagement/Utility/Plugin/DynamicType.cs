...
namespace Microsoft.PackageManagement.Internal.Utility.Plugin
{
    ...
    internal class DynamicType
    {
        ...
        private Type _type;
        private readonly MethodInfo OnUnhandledException;
        ...
        internal static object Create(Type tInterface, OrderedDictionary<Type, List<MethodInfo, MethodInfo>> instanceMethods, List<Delegate, MethodInfo> delegateMethods, List<MethodInfo> stubMethods, List<Type, object> usedInstances)
        {
            ...
            string key = tInterface.GetTypeInfo().Assembly.FullName + "::" + tInterface.Name + ":::" + instanceMethods.Keys.Select(each => each.GetTypeInfo().Assembly.FullName + "." + each.FullName + "." + instanceMethods[each].Select(mi => mi.Value.ToSignatureString()).JoinWithComma()).JoinWith(";\r\n") +
        ...
        private DynamicType(Type interfaceType, OrderedDictionary<Type, List<MethodInfo, MethodInfo>> methods, List<Delegate, MethodInfo> delegates, List<MethodInfo> stubs)
        {
            ...
            foreach (Type instanceType in methods.Keys)
            {
                // generate storage for object
                FieldBuilder field = _dynamicType.DefineField("_instance_{0}".format(++counter), instanceType, FieldAttributes.Private);
                _storageFields.Add(field);

                // create methods

                foreach (KeyValuePair<MethodInfo, MethodInfo> method in methods[instanceType])
                {
            ...
            foreach (KeyValuePair<Delegate, MethodInfo> d in delegates)
            {
                FieldBuilder field = _dynamicType.DefineField("_delegate_{0}".format(++counter), d.Key.GetType(), FieldAttributes.Private);
                _storageFields.Add(field);
            ...
            foreach (MethodInfo method in stubs)
            {
        ...
        {
            ConstructorInfo proxyConstructor = Type.GetConstructors()[0];
            object instance = proxyConstructor.Invoke(instances.Select(each => each.Value).Concat(delegates.Select(each => each.Key)).ToArray());
            // set the implemented methods collection
            FieldInfo imf = Type.GetField("__implementedMethods", BindingFlags.NonPublic | BindingFlags.Instance);
            if (imf != null)
            {
...
#else
            ModuleBuilder dynamicModule = _dynamicAssembly.DefineDynamicModule(_proxyName);
#endif

            // Define a runtime class with specified name and attributes.
            if (interfaceType.GetTypeInfo().IsInterface)
            {
                TypeBuilder dynamicType = dynamicModule.DefineType(_proxyName, TypeAttributes.Public, typeof(object));
                dynamicType.AddInterfaceImplementation(interfaceType);
                return dynamicType;
            }
            else
            {
                TypeBuilder dynamicType = dynamicModule.DefineType(_proxyName, TypeAttributes.Public, interfaceType);
                return dynamicType;
            }
        }
        ...
        internal void DefineConstructor(Type parentClassType)
        {
            ...
            Type[] types = _storageFields.Select(each => each.FieldType).ToArray();

            ConstructorBuilder constructor = _dynamicType.DefineConstructor(MethodAttributes.Public, CallingConventions.Standard, types);
            ...
            {
#if !CORECLR
                ConstructorInfo basector = parentClassType.GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public, null, new Type[0], null);
#else
            ...
            int index = 1;
            foreach (FieldBuilder backingField in _storageFields)
            {
