...
namespace Microsoft.PackageManagement.Internal.Utility.Extensions
{
    ...
    internal static class FilesystemExtensions
    {
        ...
        public static string CounterHex => Counter.ToString("x8", CultureInfo.CurrentCulture);
        ...
        public static void TryHardToDelete(this string location)
        {
            ...
            if (File.Exists(location) || Directory.Exists(location))
            {
                try
                {
                    ...
                    string tmpFilename = GenerateTemporaryFileOrDirectoryNameInTempDirectory() + ".delete_me"; // generates a unique filename but not a file!
        ...
        public static void ResetTempFolder()
        {
            ...
            string appName = typeof(FilesystemExtensions).GetTypeInfo().Assembly.GetName().Name;
            if (OriginalTempFolder.IndexOf(appName, StringComparison.CurrentCultureIgnoreCase) == -1)
            {
                string appTempPath = Path.Combine(OriginalTempFolder, appName);
                ...
                foreach (string directory in Directory.EnumerateDirectories(TempPath))
                {
                    try
                    {
        ...
        public static string CanonicalizePath(this string path, bool isPotentiallyRelativePath)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(pathUri.Host) && (!OSInformation.IsWindows))
                {
                    ...
                    string drive = pathUri.Segments[1].Replace('/', '\\'); // the zero segment is always just '/'

                    StringBuilder sb = new StringBuilder(512);
                    int size = sb.Capacity;

                    int error = NativeMethods.WNetGetConnection(drive, sb, ref size);
                    if (error == 0)
                    {
        ...
        public static byte[] ReadBytes(this string path, int maxLength)
        {
            if (path.FileExists())
            {
                try
                {
                    byte[] buffer = new byte[Math.Min(new FileInfo(path).Length, maxLength)];
