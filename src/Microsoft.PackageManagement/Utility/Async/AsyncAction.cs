    ...
    public abstract class AsyncAction : IAsyncAction
    {
        ...
        private TimeSpan TimeLeft
        {
            get
            {
                ...
                {
                    TimeSpan timeToAbort = _responsiveness.Subtract(DateTime.Now.Subtract(_lastActivity));
                    return timeToAbort < TimeSpan.Zero ? TimeSpan.Zero : timeToAbort;
                }

                TimeSpan timeToRespond = _responsiveness.Subtract(DateTime.Now.Subtract(_lastActivity));
                if (timeToRespond < TimeSpan.Zero)
                {
                    timeToRespond = TimeSpan.Zero;
                }

                TimeSpan timeToTimeout = _timeout.Subtract(DateTime.Now.Subtract(_callStart));
        ...
        public WaitHandle CompleteEvent => _completed.WaitHandle;
        ...
        public virtual bool IsCanceled => _cancellationTokenSource.IsCancellationRequested;

        public bool IsAborted => _actionState == ActionState.Aborting || _actionState == ActionState.Aborted;
